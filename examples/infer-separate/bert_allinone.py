...
default_cache_path = os.path.join(torch_cache_home, 'bert_torch')
...
def load_vocab(vocab_file):
    ...
    reverse_vocab = collections.OrderedDict()
    index = 0
...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_file, do_lower_case=True, max_len=None, do_basic_tokenize=True,
        ...
        self.max_len = int(1e12) if not max_len else max_len
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_path, cache_dir=None, *inputs, **kwargs):
        """
        Instantiate a PreTrainedBertModel from a pre-trained model file.
        """
        vocab_file = pretrained_model_path
...
class BertForSequenceClassificationContextDeep(BertPreTrainedModel):

    def __init__(self, config, num_labels):
        super(BertForSequenceClassificationContextDeep, self).__init__(config)
...
def inference(data_dir, bert_model, task_name, output_path, max_seq_length, do_lower_case, infer_batch_size=8):
    ...
    processor, output_mode = (processors[task_name](), output_modes[task_name])
    ...
    logger.info("  Batch size = %d", infer_batch_size)
    ...
    test_dataloader = DataLoader(test_data, sampler=test_sampler, batch_size=infer_batch_size)

    preds = []
    for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader, desc="Inferencing"):
